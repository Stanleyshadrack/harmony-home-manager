...
export default function PropertyDetail() {
  ...
  const propertyUnits = useMemo(
    () => units.filter((u) => u.propertyId === id),
    [units, id]
  );

  /* =========================
     Derived stats (SOURCE OF TRUTH)
  ========================= */
  ...
  return (
    ...
    >
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4 mb-6">
        <Stat title="Total Units" value={totalUnits} subtitle={`${occupiedUnits} occupied, ${vacantUnits} vacant`} icon={<Home />} />
        <Stat title="Occupancy Rate" value={`${occupancyRate}%`} progress={occupancyRate} icon={<Users />} />
        <Stat title="Potential Income" value={`KES ${totalMonthlyRent.toLocaleString()}`} subtitle={`Current: KES ${currentIncome.toLocaleString()}`} icon={<DollarSign />} />
        <Stat title="Property Type" value={property.propertyType} subtitle={`Created ${new Date(property.createdAt).toLocaleDateString()}`} icon={<Building2 />} />
      </div>
          ...
          {propertyUnits.length === 0 ? (
            ...
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
              {propertyUnits.map((unit) => (
                <UnitCard key={unit.id} unit={unit} getStatusColor={getStatusColor} />
              ))}
            </div>
          )}
        </TabsContent>
              ...
              {property.amenities.length === 0 ? (
                ...
                <div className="grid gap-3 md:grid-cols-3">
                  {property.amenities.map((a) => (
                    <div key={a} className="flex items-center gap-2 p-3 bg-muted/50 rounded">
                      {amenityIcons[a.toLowerCase()] ?? <Building2 className="h-4 w-4" />}
